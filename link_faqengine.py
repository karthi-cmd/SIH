    ...
    def query(self, usr):
        ...
        try:
            ...
            # if len(cos_sims) > 0:
                ...
                x=self.data['Answer'][questionset.index[ind]]
                print(x)
                text=str(x)
                print(text)
                ...
                print(y)
                if y>0:
                    return("please find your relevant answer : "+ urls[y-1])
                else:
                    return self.data['Answer'][questionset.index[ind]]
...
if __name__ == "__main__":
    ...
    faqslist = [os.path.join(base_path,"UGC_2.csv"),os.path.join(base_path,"UGC_3.csv")]
    faqmodel = FaqEngine(faqslist, 'tfidf')
    response = faqmodel.query("What is the EXAM PATTERN for NTA UGC NET 2021 Exam?")
    print(response)
