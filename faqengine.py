...
if __name__ == "__main__":
    ...
    faqslist = [os.path.join(base_path,"Greetings.csv"), os.path.join(base_path,"test2.csv")]
    faqmodel = FaqEngine(faqslist, 'tfidf')
    a=input("enter your question : ")
    response = faqmodel.query(a)
    print(response)
    i=int(input("enter 1/0"))
    if i==0:
        data=pd.read_csv("data/bertcheck.csv")   
        #df = [pd.read_csv(csvfile).dropna() for csvfile in faqslist]
        #data = pd.concat(df, ignore_index=True)
        d=(str(data))
        print(type(d))
        print(d)
        ques=pipeline("question-answering",model='distilbert-base-uncased-distilled-squad')
        para=d
        result=ques(question=a,context=d)
